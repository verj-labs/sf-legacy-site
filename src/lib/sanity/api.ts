...
interface SanityVehicle {
  ...
  model: string
  trim?: string
  bodyType: "sedan" | "suv" | "truck" | "coupe" | "convertible" | "wagon" | "hatchback" | "van" | "crossover"
  odometer: number
  price: number
  salePrice?: number
  transmission: string
...
const transformSanityVehicle = (sanityVehicle: SanityVehicle): Vehicle => {
  return {
    ...
    slug: typeof sanityVehicle.slug === 'object' ? sanityVehicle.slug?.current : sanityVehicle.slug,
...
export async function getAllVehicles(): Promise<Vehicle[]> {
  console.log('🔍 getAllVehicles called - USE_SANITY_DATA:', USE_SANITY_DATA)
  console.log('🔍 Environment variable NEXT_PUBLIC_USE_SANITY_DATA:', process.env.NEXT_PUBLIC_USE_SANITY_DATA)
  ...
  try {
    console.log('🔄 Fetching vehicles from Sanity...')
...
export async function getFeaturedVehicles(): Promise<Vehicle[]> {
  ...
  try {
    console.log('🔄 Fetching featured vehicles from Sanity...')
...
export async function getVehicleById(id: string): Promise<Vehicle | null> {
  if (!USE_SANITY_DATA) {
    console.log(`📦 Using mock data for vehicle ID: ${id}`)
    return mockVehicles.find((v: Vehicle) => v._id === id) || null
  }

  try {
    console.log(`🔄 Fetching vehicle ${id} from Sanity...`)
...
export async function getVehicleBySlug(slug: string): Promise<Vehicle | null> {
  ...
  try {
    console.log(`🔄 Fetching vehicle with slug '${slug}' from Sanity...`)
    const vehicle = await sanityClient.fetch(VEHICLE_BY_SLUG_QUERY, { slug })
    return vehicle ? transformSanityVehicle(vehicle) : null
  } catch (error) {
    console.error(`❌ Error fetching vehicle with slug '${slug}' from Sanity:`, error)
    console.log('📦 Falling back to mock data')
    return mockVehicles.find((v: Vehicle) => v.slug === slug) || null
  }
}
...
export async function getVehiclesByMake(make: string): Promise<Vehicle[]> {
  if (!USE_SANITY_DATA) {
    ...
    return mockVehicles.filter((v: Vehicle) => v.make.toLowerCase() === make.toLowerCase())
  }

  try {
    console.log(`🔄 Fetching vehicles by make '${make}' from Sanity...`)
    const vehicles = await sanityClient.fetch(VEHICLES_BY_MAKE_QUERY, { make })
    return vehicles.map(transformSanityVehicle)
  } catch (error) {
    console.error(`❌ Error fetching vehicles by make '${make}' from Sanity:`, error)
    console.log('📦 Falling back to mock data')
    return mockVehicles.filter((v: Vehicle) => v.make.toLowerCase() === make.toLowerCase())
  }
}
...
export async function getVehiclesByBodyType(bodyType: string): Promise<Vehicle[]> {
  if (!USE_SANITY_DATA) {
  ...
  try {
    console.log(`🔄 Fetching vehicles by body type '${bodyType}' from Sanity...`)
    const vehicles = await sanityClient.fetch(VEHICLES_BY_BODY_TYPE_QUERY, { bodyType })
    return vehicles.map(transformSanityVehicle)
  } catch (error) {
    console.error(`❌ Error fetching vehicles by body type '${bodyType}' from Sanity:`, error)
    console.log('📦 Falling back to mock data')
    return mockVehicles.filter((v: Vehicle) => v.bodyType === bodyType)
  }
}
...
export async function getRelatedVehicles(make: string, excludeId: string): Promise<Vehicle[]> {
  if (!USE_SANITY_DATA) {
    console.log(`📦 Using mock data for related vehicles`)
    return relatedVehicles.filter((v: Vehicle) => v._id !== excludeId).slice(0, 4)
  }

  try {
    console.log(`🔄 Fetching related vehicles for make '${make}' from Sanity...`)
    const vehicles = await sanityClient.fetch(RELATED_VEHICLES_QUERY, { make, excludeId })
    return vehicles.map(transformSanityVehicle)
  } catch (error) {
...
export async function getVehicleMakes(): Promise<string[]> {
  if (!USE_SANITY_DATA) {
    ...
    const makes = [...new Set(mockVehicles.map((v: Vehicle) => v.make))].sort() as string[]
    return makes
  }

  try {
    console.log('🔄 Fetching vehicle makes from Sanity...')
...
export async function getVehicleBodyTypes(): Promise<string[]> {
  if (!USE_SANITY_DATA) {
    ...
    const bodyTypes = [...new Set(mockVehicles.map((v: Vehicle) => v.bodyType))].sort() as string[]
    return bodyTypes
  }

  try {
    console.log('🔄 Fetching vehicle body types from Sanity...')
...
export async function getVehiclesByPriceRange(minPrice: number, maxPrice: number): Promise<Vehicle[]> {
  const allVehicles = await getAllVehicles()
